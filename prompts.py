...
반드시 아래의 JSON 형식에 맞추어 답변해야 하며, 다른 텍스트는 출력하지 마세요.
컴포넌트 타입('component_type')은 화면의 요소에 가장 잘 어울리는 HDS Storybook 컴포넌트 이름을 PascalCase로 자유롭게 지어주세요.
위치(x_percent, y_percent)와 크기(width_percent, height_percent)는 0.0 에서 1.0 사이의 실수로 표현하세요.
...
def get_storybook_docs():
    """외부 JSON 파일(storybook_docs.json)에서 Storybook 명세를 실시간으로 읽어옵니다."""
    file_path = "storybook_docs.json"
    default_docs = {
        ...
        "HdsModal": "<HdsModal isOpen={isOpen} onClose={closeHandler} title=''> 팝업 모달입니다."
    }
    ...
    if os.path.exists(file_path):
        try:
            ...
            logger.error(f"storybook_docs.json 파일을 읽는 중 문법 오류가 발생했습니다. (수정 내역을 확인하세요): {e}")
            return default_docs
    else:
        # 파일이 없으면 초기 샘플 JSON 파일을 자동 생성합니다.
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(default_docs, f, ensure_ascii=False, indent=4)
        except Exception as e:
            logger.error(f"storybook_docs.json 샘플 파일 자동 생성 실패: {e}")
        return default_docs
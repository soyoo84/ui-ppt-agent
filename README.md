...
4. **프론트엔드 코드 및 엑셀(CSV) 명세서 추출**
   * 프론트엔드 개발자가 즉시 활용할 수 있는 HTML/React(JSX) 코드를 생성하며, 기획자를 위해 추출된 컴포넌트 목록을 엑셀과 호환되는 CSV 파일로 제공합니다.
5. **채팅형(Chat UI) 피드백 및 클립보드 붙여넣기 지원**
   * 번거로운 파일 업로드 없이 클립보드 복사/붙여넣기(Ctrl+V)를 지원하며, 1차 생성 후 하단 채팅창에 피드백(예: "로그인 버튼 파란색으로 바꿔줘")을 대화하듯 입력하여 결과물을 실시간으로 갱신할 수 있습니다.
...
1. **요청 단계**: 사용자가 클립보드(Ctrl+V) 또는 파일 업로드를 통해 이미지를 추가하고, 시스템 프롬프트(HTML or React)를 선택합니다.
...
4. **결과 확인 및 다운로드**: 미리보기 화면이 렌더링되며, 5가지(PPT, HTML, CSS, JSON, CSV) 개별 파일 다운로드 버튼이 활성화됩니다.
5. **피드백 (Chat)**: 하단 채팅창에 수정 사항을 입력하면 이전 결과(Context)를 바탕으로 AI가 즉시 결과물을 갱신합니다.
